...
@section('content')
    ...
    <div class="box">
        ...
        <div class="box-body">
            ...
            <div class="row">
               <div class="col-md-6">
                   <h3>{{ __('admin.developer_pdf') }}</h3>
                   @if($project->developer_pdf)
                       @foreach(json_decode($project->developer_pdf) as $pdf)
                           <a href="{{ url('uploads/'.$pdf) }}" target="_blank"> developer pdf </a><br>
                       @endforeach
                   @endif
               </div>
               <div class="col-md-6">
                   <h3>{{ __('admin.broker_pdf') }}</h3>
                   @if($project->broker_pdf)
                       @foreach(json_decode($project->broker_pdf) as $pdf)
                           <a href="{{ url('uploads/'.$pdf) }}" target="_blank"> developer pdf </a><br>
                       @endforeach
                   @endif
               </div>

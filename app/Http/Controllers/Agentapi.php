...
class Agentapi extends Controller
{
    ...
    public function getCaller(Request $request)
    {
        ...
        if ($lead) {
            ...
            // ];
            return ['status' => 'ok', 'name' => @$lead->first_name . ' ' . @$lead->last_name, 'image' => $lead->image, 'type' => $data, 'lead_data' => $full_info, 'projects' => $pros];
        } else {
    ...
    public function get_lead(Request $request, $lead_id = 0)
    {
        ...
        if ($lead) {
            ...
            $data = ['id' => $lead->id, 'first_name' => $lead->first_name, 'last_name' => $lead->last_name, 'image' => $lead->image, 'phone' => $lead->phone, 'lead_source' => @$lead->source->name, 'reference' => $lead->reference, 'title' => @$lead->title->name, 'industry' => @$lead->industry->name, 'email' => $lead->email, 'status' => @$seen, 'created_by' => @$lead->user->name, 'created_at' => @$lead->created_at->format('Y-m-d H:i:s'), 'updated_at' => @$lead->updated_at->format('Y-m-d H:i:s'), 'r_agent' => ['name' => @$lead->agent->name, 'image' => @$lead->agent->image, 'type' => @$lead->agent->agentType->name], 'c_agent' => ['name' => @$lead->commercialAgent->name, 'image' => @$lead->commercialAgent->image, 'type' => @$lead->commercialAgent->agentType->name], 'calls' => $calls, 'meetings' => $meetings, 'voice_notes' => $voice_notes, 'notes' => $notes, 'requests' => $requests, 'documents' => @$lead->documents];

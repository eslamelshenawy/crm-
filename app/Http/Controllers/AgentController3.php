...
class AgentController3 extends Controller
{
    ...
    {
        $request     = json_decode($request->getContent());
        $switch      = AdminNotification::where('assigned_to', $request->user_id)->where('type', 'switch')->orWhere('type', 'note_lead')->orderBy('created_at', 'desc')->take(10)->get();
        ...
        foreach ($notes as $row) {
            ...
            array_push($data, array(
                'type'         => $type,
                'title'        => User::find($row->user_id)->name . ' add a note in lead "' . @Lead::find($row->assigned_to)->first_name . ' ' . @Lead::find($row->assigned_to)->first_name . '" to you',
                'id'           => @$lead->id,
                'name'         => @$lead->first_name . ' ' . @$lead->last_name,
                'phone'        => @$lead->phone,
                'email'        => @$lead->email ? $lead->email : '',

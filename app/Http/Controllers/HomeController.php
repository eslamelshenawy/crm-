	...
	public function search(Request $request) {
		...
		if ('project' == $type) {
			...
			foreach ($projects as $row) {
				...
				$location = Location::find($row->location_id)->{$lang . '_name'};
				array_push($data, ["id" => $row->id, "title" => $row->{$lang . '_name'}, 'price' => $row->meter_price,
			...
			foreach ($resales as $resale) {
				...
				array_push($data, ['id' => $resale->id, 'location' => $resale->{'location_' . $lang . '_name'},
			...
			foreach ($rentals as $rental) {
				...
				array_push($data, ['id' => $rental->id, 'location' => $rental->{'location_' . $lang . '_name'},
...
';
	...
	public function dashboard(Request $request) {
		...
		$event = ProdcastEvent::all();
		$event_note = null;
		$check_user = ProdeventStatus::where('user_id', Auth()->User()->id)->get();
		$callArray=array();
		$mettingArray=array();
		...
		if (auth()->user()->type == 'admin') {
			...
			foreach ($check_user as $key => $value_check) {
				foreach ($event as $key => $value) {
					$event_note = ProdcastEvent::whereDate('date_event', '>=', Carbon::now()->toDateString())->where('id', $value_check->prot_event_id)->get();
				}
			}
			$month = date('Y-m');
			$day = date('Y-m-d');
			...
			$agents = User::where('type', '!=', 'employee')->get();
			$agentData = [];
			...
			$projects = Project::has('request')->withcount('request')->where('created_at', '>', $request->startDate)
				->where('created_at', '<', $request->endDate)->orderBy('request_count', 'desc')->limit(10)->get();
			$projectSumLeads = \App\Request::all()->count();

			foreach(Project::all() as $k=>$project){
				if(\App\Call::where('projects','like',"%$project->id%")->count() > 0 || \App\Meeting::where('projects','like',"%$project->id%")->count() > 0){
				$callArray[$k]['project']=$project->en_name.$project->id;
				$callArray[$k]['count']=\App\Call::where('projects','like',"%$project->id%")->count();
				$callArray[$k]['val']=\App\Call::where('projects','like',"%$project->id%")->get();

				$mettingArray[$k]['project']=$project->en_name;
				$mettingArray[$k]['count']=\App\Meeting::where('projects','like',"%$project->id%")->count();
				$mettingArray[$k]['val']=\App\Meeting::where('projects','like',"%$project->id%")->get();
				}
			}
			...
			$callStatus = $callStatus->has('calls')->withCount(['calls' => function ($q) use ($request) {
				$q->where('created_at', '>', $request->startDate)->where('created_at', '<', $request->endDate);
			}])->get();

			$groups = \App\Group::with('groupMembers')->with('children')->get();
			$count = 0;
			$leadTeamCount = [[]];
			...
			if (\App\Group::where('team_leader_id', auth()->user()->id)->count() > 0) {
				...
				foreach(Project::all() as $k=>$project){
					$scall=\App\Call::whereHas('lead', function ($q) use ($ids) {$q->whereIn('agent_id', $ids)->orWhereIn('commercial_agent_id', $ids);})->where('projects','like',"%$project->id%");
					$smeeting=\App\Meeting::whereHas('lead', function ($q) use ($ids) {$q->whereIn('agent_id', $ids)->orWhereIn('commercial_agent_id', $ids);})->where('projects','like',"%$project->id%");
					if($scall->count() > 0 || $smeeting->count() > 0){
						$callArray[$k]['project']=$project->en_name.$project->id;
						$callArray[$k]['count']=$scall->count();
						$callArray[$k]['val']=$scall->get();

						$mettingArray[$k]['project']=$project->en_name;
						$mettingArray[$k]['count']=$smeeting->count();
						$mettingArray[$k]['val']=$smeeting->get();
					}
				}
				$proMeetingCall=['meetings'=>$mettingArray,'calls'=>$callArray];

				$projectSumLeads = \App\Request::whereHas('lead', function ($q) use ($ids) {$q->whereIn('agent_id', $ids)->orWhereIn('commercial_agent_id', $ids);})->count();
				...
				foreach ($groupMembers->get() as $k => $member) {
				...
				$projects = Project::has('request')->withCount([
					...
					->where('created_at', '<', $request->endDate)->orderBy('request_count', 'desc')->limit(10)->get();
				$projectSumLeads = \App\Request::whereHas('lead', function ($q) use ($memberr) {$q->where('agent_id', $memberr)->orWhere('commercial_agent_id', $memberr);})->count();
				foreach(Project::all() as $k=>$project){
					$scall=\App\Call::whereHas('lead', function ($q) use ($memberr) {$q->where('agent_id', $memberr)->orWhere('commercial_agent_id', $memberr);})->where('projects','like',"%$project->id%");
					$smeeting=\App\Meeting::whereHas('lead', function ($q) use ($memberr) {$q->where('agent_id', $memberr)->orWhere('commercial_agent_id', $memberr);})->where('projects','like',"%$project->id%");
					if($scall->count() > 0 || $smeeting->count() > 0){
						$callArray[$k]['project']=$project->en_name.$project->id;
						$callArray[$k]['count']=$scall->count();
						$callArray[$k]['val']=$scall->get();
				...
				$callStatus = new \App\CallStatus;
				$callStatus = $callStatus->has('calls')->withCount(['calls' => function ($q) use ($request, $memberr) {
